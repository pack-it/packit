...
Where the `<PACKAGE-NAME>` is the name of the package to switch and `<VERSION>` is the new active version. To learn more about active versions checkout [active packages](../structure.md#active-packages).
...
This is a complete list of all flags which can be used with the `pit switch` command.
