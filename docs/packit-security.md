...
Packit is a package manager and therefore responsible for installing packages on a users system that run under the permissions of this user. Packit cannot guarantee that software contained in a trusted package is harmless. By being open in the security design choices of Packit, we hope to work towards a more secure ecosystem.
<br>
This security model assumes that repositories and their maintainers are trusted. Users need to make sure they trust the repositories they are using, as well as their maintainers.
...
On Unix we specifically recommend not using `sudo` when running Packit, as this is not required. On Windows Administrator rights are needed to make sure symlinks can be created.
Running with `sudo` results in the scripts executing with root privileges, this is a serious risk as this could contain harmful code if unsecure repositories are used.
...
- Manual review of package scripts by the repository maintainers, to prevent malicious code inside package scripts.
...
### Denying sudo permissions
To ensure the scripts do not run with root privileges, Packit can deny running with `sudo`. This would then mean only normal user accounts are able to run Packit. This does still require good consideration of the behaviour on Windows, since Windows still requires Administrator rights to create symlinks. A possible solution for these rights on Windows is looking into alternatives for symlinks, or ensure symlinks can be created without Administrator rights.
