...
/// Returns an `ExitCode`.
fn main() -> ExitCode {
    // Execute setup, which will partially determine how the cleanup is executed
    let _cleanup = setup();
...
fn setup() -> Cleanup {
    ...
    let register_file = prefix.join("Register.toml");
    ...
    if prefix.exists() {
        eprintln!("Test prefix '{}' specified in 'TestConfig.toml' already exists", prefix.display());
        exit(1);
    }
    ...
    if let Err(e) = fs::write(&register_file, "# DO NOT EDIT THIS FILE") {
        eprintln!("Failed to create test 'Register.toml' at '{}'", register_file.display());
        eprintln!("Error: {e}");
        exit(1);
    }
