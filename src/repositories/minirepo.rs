...
const MINIREPO_MAINTAINER: &str = concat!("Packit v", env!("CARGO_PKG_VERSION"));

/// The errors that occur during minirepo creation.
#[derive(Error, Debug)]
...
pub type Result<T> = std::result::Result<T, MiniRepoError>;

/// The MiniRepoCreator, managing the creation of mini repositories.
pub struct MiniRepoCreator<'a> {
    repository_manager: &'a RepositoryManager<'a>,
    target: Target,
}

impl<'a> MiniRepoCreator<'a> {
    /// Creates a new MiniRepoCreator.
    ...
    pub fn create_minirepo(&self, included_packages: HashSet<PackageId>, destination: &PathBuf) -> Result<()> {
        ...
        for package_id in included_packages {
            ...
            if package_index.contains_key(&package_id.name) && package_index[&package_id.name] != repository_id {
                return Err(MiniRepoError::PackageFromMultipleRepositories {
            ...
            if prebuild_url.is_none() {
                return Err(MiniRepoError::PrebuildNotFound { package_id });
            }
        ...
        let repository_meta = RepositoryMeta {
            name: "generated-minirepo".into(),
            description: format!("A minirepo generated by Packit, containing {} packages", package_index.len()),
            maintainers: vec![MINIREPO_MAINTAINER.into()],
    ...
    fn download_file(
        ...
        let contents = match contents {
            ...
            None => {
                return Err(MiniRepoError::FileNotFound {
