...
impl Verifier {
    ...
    pub fn next_check(&mut self, packages: &Vec<PackageId>, register: &PackageRegister, config: &Config) -> Result<Option<Issue>> {
        ...
        let issue = match check {
            ...
            Check::InvalidDependencies => package::check_invalid_dependencies(packages, register, config)?,
            ...
            Check::ForbiddenLink => package::check_forbidden_link(packages, register, config)?,
